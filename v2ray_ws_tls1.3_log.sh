...
if [ "$release" == "centos" ]; then
    ...
    if [ -n "$(grep ' 7\.' /etc/redhat-release)" ] ;then
        ...
        rpm -Uvh http://nginx.org/packages/centos/8/x86_64/RPMS/nginx-1.24.0-1.el8.ngx.x86_64.rpm
    fi
    green "开始安装nginx编译依赖"
    yum install -y libtool perl-core zlib-devel
    yum install -y gcc pcre --allowerasing
    ...
    apt-get update
    green "开始安装nginx编译依赖"
    apt-get install -y build-essential libpcre3 libpcre3-dev zlib1g-dev liblua5.1-dev libluajit-5.1-dev libgeoip-dev google-perftools libgoogle-perftools-dev
...
EOF
    ...
    ~/.acme.sh/acme.sh  --issue  -d $your_domain -d www.$your_domain --standalone
...
function install_v2ray(){
    ...
    rm -f config.json
    wget https://raw.githubusercontent.com/Ericfengshi/v2ray-ws-tls/master/config.json
